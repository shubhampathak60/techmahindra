...
const data = {};
const researchs = awards.forEach((v) => {
  data[v.name] = v.research;
});

const getWinners = (arr, name, total) => {
  return [...arr, name];
};

let result;
const prizesOutput = awards.forEach((award) => {
  if (!result || !result[award.category + award.year]) {
    if (!result) result = {};
    result[award.category + award.year] = {
      category: award.category,
      year: award.year,
      winners: [{ name: award.name, share: 1 }],
    };
  } else if (result[award.category + award.year]) {
    result[award.category + award.year] = {
      ...result[award.category + award.year],
      winners: getWinners(
        result[award.category + award.year].winners,
        { name: award.name },
        1
      ),
    };
